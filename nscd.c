...
enum {
	HOSTS_NO_DNS, /* hosts: files myhostname // dns doesn't appear */
	HOSTS_FILES_DNS, /* hosts: files dns // files appears before dns */
	HOSTS_DNS, /* hosts: dns // files doesn't appears before dns */
};
...
typedef struct hst_response_header {
	uint32_t version_or_size;
	int32_t found; /* reused for TTL: 0 ~ not found; 1 ~ found without TTL; >=2 ~ (TTL + 2) */
	int32_t h_name_len;
...
static hst_response_header *marshal_hostent_by_dns(unsigned char *answer, ssize_t anslen)
{
	char *p;
	hst_response_header *resp;
	char h_name[MAXCDNAME];
	unsigned h_name_len;
	unsigned h_aliases_cnt = 0;
	unsigned h_addr_list_cnt = 0;
	unsigned sz = sizeof(*resp);
	int is_v6 = 0;
	uint32_t ttl = 0;
	ns_msg handle;
	...
	if (anslen > 0) {
		...
		cnt = ns_msg_count(handle, ns_s_an);
		int flag = 0;
		for (int i = 0; i < cnt; i++) {
			ns_rr rr;
			ns_parserr(&handle, ns_s_an, i, &rr);
			if (ns_rr_type(rr) == T_AAAA) is_v6 = 1;
			if (ns_rr_type(rr) == T_A || ns_rr_type(rr) == T_AAAA) {
				if (!flag) {
					strcpy(h_name, ns_rr_name(rr));
					sz += h_name_len = strsize_aligned4(h_name);
					flag = 1;
				}
				h_addr_list_cnt++;
				sz += ns_rr_rdlen(rr);
				...
				h_aliases_cnt++;
				sz += strsize(ns_rr_name(rr));
			}
			sz += h_aliases_cnt * 4;
			/* use minimium ttl */
			if (!ttl) {
				ttl = ns_rr_ttl(rr);
			} else if (ns_rr_ttl(rr) < ttl) {
				ttl = ns_rr_ttl(rr);
			}
		}
	}
	...
	resp->h_addrtype = is_v6 ? AF_INET6 : AF_INET;
	resp->h_length = is_v6 ? 16 : 4;
...
/* uint32_t h_aliases_len[h_aliases_cnt]; */
	for (int i = 0; i < cnt; i++) {
		ns_rr rr;
		ns_parserr(&handle, ns_s_an, i, &rr);
		if (ns_rr_type(rr) == T_CNAME) {
			*(uint32_t*)p = strsize(ns_rr_name(rr));
			p += 4;
		}
	}
/* char h_addr_list[h_addr_list_cnt][h_length]; */
	for (int i = 0; i < cnt; i++) {
		...
		ns_parserr(&handle, ns_s_an, i, &rr);
		if (ns_rr_type(rr) == T_A || ns_rr_type(rr) == T_AAAA) {
			memcpy(p, ns_rr_rdata(rr), ns_rr_rdlen(rr));
			p += ns_rr_rdlen(rr);
		}
	}
/* char h_aliases[h_aliases_cnt][h_aliases_len[i]]; */
	for (int i = 0; i < cnt; i++) {
		...
		ns_parserr(&handle, ns_s_an, i, &rr);
		if (ns_rr_type(rr) == T_CNAME) {
			strcpy(p, ns_rr_name(rr));
			p += strsize(ns_rr_name(rr));
		}
	}
	log(L_DEBUG, "sz:%u realsz:%u", sz, p - (char*)resp);
	return resp;
...
static ai_response_header *obtain_addrinfo_by_dns(const char *hostname)
{
	char *p, *family;
	...
	uint32_t ttl = 0;
	char answer[MAXMSGSZ], answer6[MAXMSGSZ], canonname[MAXCDNAME];
	int ret = res_search(hostname, C_IN, T_A, answer, sizeof(answer));
	int ret6 = res_search(hostname, C_IN, T_AAAA, answer6, sizeof(answer6));
	ns_msg handle, handle6;
	uint16_t cnt, cnt6;
	int flag = 0;
	...
	if (ret > -1) {
		ns_initparse(answer, ret, &handle);
		cnt = ns_msg_count(handle, ns_s_an);
		for (int i = cnt - 1; i >= 0; i--) {
			ns_rr rr;
			ns_parserr(&handle, ns_s_an, i, &rr);
			if (!flag && ns_rr_type(rr) == T_CNAME) {
				ns_name_uncompress(ns_msg_base(handle), ns_msg_end(handle), ns_rr_rdata(rr), canonname, sizeof(canonname));
				sz += canonlen += strsize(canonname);
				flag = 1;
			} else if (ns_rr_type(rr) == T_A) {
				naddrs++;
				addrslen += 4;
			}
			if (!ttl) {
	...
	if (ret6 > -1) {
		ns_initparse(answer6, ret6, &handle6);
		cnt6 = ns_msg_count(handle6, ns_s_an);
		for (int i = cnt6 - 1; i >= 0; i--) {
			ns_rr rr;
			ns_parserr(&handle6, ns_s_an, i, &rr);
			if (!flag && ns_rr_type(rr) == T_CNAME) {
				ns_name_uncompress(ns_msg_base(handle6), ns_msg_end(handle6), ns_rr_rdata(rr), canonname, sizeof(canonname));
				sz += canonlen += strsize(canonname);
				flag = 1;
	...
	if ((ret < 0 && ret6 < 0) || !naddrs) {
	    resp->error = EAI_NODATA;
		/*resp->found = 0;*/
		return resp;
	}
...
/* char ai_addr[naddrs][4 or 16]; */
	...
	for (int i = 0; i < cnt6; i++) {
		ns_rr rr;
		ns_parserr(&handle6, ns_s_an, i, &rr);
		if (ns_rr_type(rr) == T_AAAA) {
/* char ai_family[naddrs]; */
			*family++ = AF_INET6;
/* char ai_addr[naddrs][4 or 16]; */
			memcpy(p, ns_rr_rdata(rr), 16);
			p += 16;
		}
	}
/* char ai_canonname[canonlen]; */
	if (canonlen) {
		strcpy(family, canonname);
	}
	log(L_DEBUG, "sz:%u realsz:%u", sz, family + strsize(canonname) - (char*)resp);
		...
		if (CACHED_ENTRY(cached) && cached != NULL) {
			...
			if (srv == -1 || srv == csrv) {
				if (free_all) {
					...
					unsigned long ttl = (resp->found > 1 ? (resp->found - 2) * 1000 : resp->found == 1 ? config.pttl[csrv] : config.nttl[csrv]);
					if (age >= ttl) {
...
static int create_and_feed_worker(user_req *ureq)
{
	...
	if (!pid) { /* child */
		...
		argv[2] = "\0";
		argv[3] = NULL;
		if (ureq->type == GETHOSTBYNAME || ureq->type == GETHOSTBYNAMEv6 || ureq->type == GETAI) {
			switch (config.nsswitch_hosts) {
				case HOSTS_NO_DNS:
					sprintf(type, "%d", WORKER_TYPE_NATIVE);
					argv[2] = type;
					break;
				case HOSTS_DNS:
					sprintf(type, "%d", WORKER_TYPE_DNS);
					argv[2] = type;
					break;
				case HOSTS_FILES_DNS:;
					char *pi;
					int flag = 0;
					for (pi = g_hosts_domains.data; *pi; pi+= MAXCDNAME) {
						if (!strcmp(pi, ureq->reqbuf)) {
							flag = 1;
							break;
						}
					}
		...
		execve(self_exe_points_to, argv, argv+3);
		xexecve("/proc/self/exe", argv, argv+3);
	}
...
static void worker(const char *param, const char *type) NORETURN;
static void worker(const char *param, const char *type)
{
	user_req ureq;
	void *resp;
	...
	if (ureq.type == GETHOSTBYNAME
	 || ureq.type == GETHOSTBYNAMEv6
		...
		switch (atoi(type)) {
			...
			default:;
				char answer[MAXMSGSZ];
				int ret;
				// DO NOT use T_ANY here because GETHOSTBYNAME/2 does NOT mix A/AAAA records
				if (ureq.type == GETHOSTBYNAME) {
					ret = res_search(ureq.reqbuf, C_IN, T_A, answer, sizeof(answer));
				} else {
					ret = res_search(ureq.reqbuf, C_IN, T_AAAA, answer, sizeof(answer));
				}
				resp = marshal_hostent_by_dns(answer, ret);
				break;
		}
	 ...
	 || ureq.type == GETHOSTBYADDRv6
		...
		switch (atoi(type)) {
			...
			case WORKER_TYPE_DNS:
			default:
				resp = obtain_addrinfo_by_dns(ureq.reqbuf);
				break;
		}
...
static void parse_nsswitch_conf() {
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		while (*p) {
			*skip_non_whitespace(p) = '\0';
			...
			if (!strcmp(p, "dns")) {
				config.nsswitch_hosts = flag ? HOSTS_FILES_DNS : HOSTS_DNS;
				fclose(fp);
				return;
			}
	...
	fclose(fp);
	config.nsswitch_hosts = HOSTS_NO_DNS;
	return;
}

static void parse_hosts() {
	if (!g_hosts_domains.data) {
		g_hosts_domains.cnt = HOSTNAMES_INC;
		g_hosts_domains.data = xzalloc(g_hosts_domains.cnt * MAXCDNAME);
		log(L_DEBUG2, "malloc hosts cache of %u entries", g_hosts_domains.cnt);
	}
	...
	while (fgets(buf, sizeof(buf), fp) != NULL) {
		...
		*skip_non_whitespace(p) = '\0';
		...
		p = skip_whitespace(p);
		while (*p) {
			*skip_non_whitespace(p) = '\0'; // hostname
			len = strlen(p);
			memcpy(g_hosts_domains.data + (cnt++) * MAXCDNAME, p, len + 1 > MAXCDNAME ? MAXCDNAME : len + 1);
			if (cnt >= g_hosts_domains.cnt) {
				size_t newcnt = g_hosts_domains.cnt + HOSTNAMES_INC;
				g_hosts_domains.data = xrealloc(g_hosts_domains.data, newcnt * MAXCDNAME);
				memset(g_hosts_domains.data + g_hosts_domains.cnt * MAXCDNAME, 0, (newcnt - g_hosts_domains.cnt) * MAXCDNAME);
				g_hosts_domains.cnt = newcnt;
				log(L_DEBUG2, "realloc hosts cache to %u entries", newcnt);
			}
			p = skip_whitespace(p + len + 1);
		}
	}
	fclose(fp);
	return;
